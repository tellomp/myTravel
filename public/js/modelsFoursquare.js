...
function getResults(city, interest) {
    $.get("/modelsFoursquare", {city:city,interest:interest}, function(data) {
        if($("#fs_container").html().trim().length>0) {
            $("#fs_container").slick('unslick');
            $("#fs_container").empty();
        }
        ...
        for (var i=0; i<jsonData.length; i++){
            ...
            newImage.attr("src", "/images/" + category + selectImage + ".jpg");
...
/* Look for any elements with the class "searchSelect": */
